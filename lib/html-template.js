...
generateHTMLPage = (teamProfiles) => {
    ...
    `;
    const holdHTMLEngineerWithInter = [`
    </div>
    </div>
</div>
<!-- intern -->
...
crossorigin="anonymous"></script>
    ...
    for (let i = 1; i < teamProfiles.length; i++) {
        let teamMemberHTML = ``;
        if (teamProfiles[i].role === "Manager") {
                    ...
                    <div class="card-section border rounded p-3">
                        <div class="card-header-first rounded pb-5">
                            ...
                            <h4 class="card-header-title text-white"><span><i
                                        ...
                                        aria-hidden="true"></i> ${teamProfiles[i].rol}</span></h4>
                        </div>
            ...
            holdManager.push(teamMemberHTML);
        }
            ...
            holdEngineers.push(teamMemberHTML);
        }
            ...
            holdInterns.push(teamMemberHTML);
        };
    };

    // holdHTMLPages.push(htmlHeaderAndTitle);

    return htmlHeaderAndTitle;
};
